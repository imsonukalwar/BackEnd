...
apirouter.put("/update",userOuth,async(req,res)=>{
    ...
    try {
        //“Cookie se token lo, usko secret key se verify karo, aur uske andar ka data (payload) nikaal lo.”
    const payload=jwt.verify(req.cookies.token,process.env.SERVER_KEY);
    // “Token ke andar jo user ID hai, us user ka data database me update kar do.”
...
try {
    ...
    // It tells the browser to delete the cookie named token
